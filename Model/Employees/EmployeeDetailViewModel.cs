...
namespace HRCentral.Web.Models.Employees
{
    public class EmployeeDetailViewModel
    {
        ...
        [RegularExpression(@"^[a-zA-Z]*$", ErrorMessage = "FirstName must be only Characters.")]
        public string FirstName { get; set; }
        [Display(Name = "Maiden Name")]
        [RegularExpression(@"^[a-zA-Z]*$", ErrorMessage = "Middle Name must be only Characters.")]
        [StringLength(25, MinimumLength = 3, ErrorMessage = "Middle Name must be atleast 3 characters long.")]
        public string MaidenName { get; set; }
        [RegularExpression(@"^[a-zA-Z]*$", ErrorMessage = "Maiden Name must be only Characters.")]
        [StringLength(25, MinimumLength = 3, ErrorMessage = "Maiden Name must be atleast 3 characters long.")]
        public string MiddleName { get; set; }
        [Display(Name = "SurName")]
        //[Required]
        [RegularExpression(@"^[a-zA-Z]*$", ErrorMessage = "Family name must be only Characters.")]
        ...
        //[Required]
        [RegularExpression(@"^071[0-9]{7}$", ErrorMessage = "Phone number must be of a standard format e.g 0712126547.")]
        public string PersonalMobileNumber { get; set; }
        [Display(Name = "Work PhoneNumber")]
        [RegularExpression(@"^0414333[0-9]{3}$", ErrorMessage = "Phone number must be of a standard format e.g 0414333259.")]
        public string WorkMobileNumber { get; set; }
        [Display(Name = "Alternative Number")]
        //[Required]
        [RegularExpression(@"^07[0-9]{8}$", ErrorMessage = "Phone number must be of a standard format e.g 0712126547.")]
